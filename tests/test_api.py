...
def test_run_task_endpoint_success(
    ...
    # We can't easily mock FastAPI dependencies in tests
    # Just verify the endpoint works and returns a session ID
    ...
    assert "claude -p" in data["handoff_command"]
    assert data["session_id"] in data["handoff_command"]
...
def test_run_task_endpoint_no_project(test_client: TestClient, tmp_vault: Path) -> None:
    """Test POST /api/tasks/{id}/run with task missing project field."""
